...
class Management(models.Model):
    ...
    docustructure_id = fields.Many2one("mgmt.docustructure", string="Docustructure")
    document_id = fields.Many2one(
        "mgmt.document",
        string="Source",
        store=True,
        related="docustructure_id.document_id",
    )
    requirement_ids = fields.Many2many("mgmt.requirement", string="Requirement")
    ...
    state = fields.Selection(
        [("changed", "Changed"), ("incorporated", "Incorporated")],
        default="changed",
        required=True,
    )
    ...
    def button_confirmed(self):
        self.message_post(
            body=(
                "Confirmed. (The formulated requirements fully reflect the requirements of the paragraph.)"
            )
        )
