...
class Management(models.Model):
    ...
    requirement_id = fields.Many2one("mgmt.requirement", string="Requirement")
    audit_id = fields.Many2one("mgmt.audit", string="Audit")
    audit_date = fields.Datetime(
        string="Datum", store=True, related="audit_id.date_begin"
    )
    requirement_name = fields.Char(
        string="Requirement Name", related="requirement_id.name"
    )
    requirement_description = fields.Text(
        string="Requirement Description", related="requirement_id.description"
    )
    requirement_paragraph_ids = fields.Many2many(
        "mgmt.paragraph",
        string="Paragraphs",
    ...
    requirement_paragraph_display_name = fields.Char(
        string="Paragraph", store=True, related="requirement_paragraph_ids.display_name"
    )
    ...
    requirement_paragraph_document_id_id = fields.Many2one(
        "mgmt.document",
