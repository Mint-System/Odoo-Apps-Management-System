    ...
    def _check_documentation_type(self):
        for rec in self:
            if rec.type == "url":
                if not rec.url:
                    raise ValidationError(
                        "URL must be set when documentation type is 'Web link'."
                    )
                if not rec.name:
                    raise ValidationError(
                        "Name must be set when documentation type is 'Web link'."
                    )
                if rec.knowledge_article_id:
                    raise ValidationError(
                        "Knowledge article must be empty when type is 'Web link'."
                    )
            ...
            if rec.type == "wiki":
                ...
                if rec.url:
                    raise ValidationError(
                        "URL must be empty when type is 'Knowledge article'."
                    )
